        ...
        <div className="absolute right-0 mt-1 w-52 rounded-md border border-border-default bg-card shadow-md z-20">
          {FORMATS.map(({ label, format }) => (
            <button
              ...
              // Only PDF is unimplemented: export_handler returns 501 for
              // format=pdf, while docx builds the document and returns a
              // presigned URL. The blanket tooltip claimed both were pending.
              title={format === 'pdf' ? 'PDF export is not yet available' : undefined}
            >
              {label}
            </button>
          ))}
