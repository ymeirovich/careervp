...
class TestHealthReportsBedockRemoved:
    ...
    def test_health_reports_anthropic_not_bedrock(self):
        ...
        with patch("anthropic.Anthropic") as mock_cls, \
            ...
            assert "bedrock" not in body.get("services", {}), (
                f"'bedrock' key still in services: {body.get('services')}"
            )
    ...
    def test_health_response_has_anthropic_key(self):
        ...
        with patch("anthropic.Anthropic") as mock_cls, \
            ...
            assert "anthropic" in result.get("services", {}), (
                f"'anthropic' missing from services: {result.get('services')}"
            )
    ...
    def test_health_response_has_dynamodb_key(self):
        ...
        with patch("anthropic.Anthropic") as mock_cls, \
            ...
            mock_boto.return_value.describe_table.return_value = {"Table": {}}
            result = _call_health_check()
            assert "dynamodb" in result.get("services", {}), (
                f"'dynamodb' missing from services: {result.get('services')}"
            )
    ...
    def test_health_response_has_no_lambda_key(self):
        ...
        with patch("anthropic.Anthropic") as mock_cls, \
            ...
            mock_boto.return_value.describe_table.return_value = {"Table": {}}
            result = _call_health_check()
            assert "lambda" not in result.get("services", {}), (
                f"'lambda' key should not be in services: {result.get('services')}"
            )
...
class TestHealthAnthropicCheck:
    ...
    def test_health_reports_anthropic_healthy_on_success(self):
        ...
        with patch("anthropic.Anthropic") as mock_cls, \
            ...
            mock_boto.return_value.describe_table.return_value = {"Table": {}}
            result = _call_health_check()
            assert result["services"]["anthropic"] == "healthy", (
                f"Expected 'healthy', got: {result['services'].get('anthropic')}"
            )
    ...
    def test_health_reports_anthropic_degraded_on_error(self):
        ...
        with patch("anthropic.Anthropic") as mock_cls, \
            ...
            assert result["services"]["anthropic"] == "degraded", (
                f"Expected 'degraded', got: {result['services'].get('anthropic')}"
            )
...
class TestHealthDynamoDBCheck:
    ...
    def test_health_reports_dynamodb_healthy(self):
        ...
        with patch("anthropic.Anthropic") as mock_cls, \
            ...
            result = _call_health_check()
            assert result["services"]["dynamodb"] == "healthy", (
                f"Expected 'healthy', got: {result['services'].get('dynamodb')}"
            )
    ...
    def test_health_reports_dynamodb_degraded_on_error(self):
        ...
        with patch("anthropic.Anthropic") as mock_cls, \
            ...
            result = _call_health_check()
            assert result["services"]["dynamodb"] == "degraded", (
                f"Expected 'degraded', got: {result['services'].get('dynamodb')}"
            )
    ...
    def test_health_status_degraded_when_any_service_degraded(self):
        ...
        with patch("anthropic.Anthropic") as mock_cls, \
            ...
            assert result["status"] == "degraded", (
                f"Expected 'degraded' when anthropic fails, got: {result['status']}"
            )
    ...
    def test_health_status_healthy_when_all_services_healthy(self):
        ...
        with patch("anthropic.Anthropic") as mock_cls, \
            ...
            assert result["status"] == "healthy", (
                f"Expected 'healthy' when all services pass, got: {result['status']}"
            )
