...
class TestRouteSurfaceMatchesSpec:
    ...
    def test_route_count_is_30(self):
        ...
        assert len(CANONICAL_ROUTES) == EXPECTED_ROUTE_COUNT, (
            f"Expected {EXPECTED_ROUTE_COUNT} canonical routes, got {len(CANONICAL_ROUTES)}"
        )
    ...
    def test_deprecated_route_not_in_canonical_spec(self, deprecated_route):
        ...
        assert deprecated_route not in CANONICAL_ROUTES, (
            f"Deprecated route still in canonical spec: {deprecated_route}"
        )
...
class TestRouteSurfaceDiffEmpty:
    ...
    def test_route_diff_file_exists(self):
        ...
        assert os.path.exists(ROUTE_DIFF_PATH), (
            f"route-surface-diff.txt missing at {ROUTE_DIFF_PATH}"
        )
    ...
    def test_frozen_spec_exists(self):
        ...
        assert os.path.exists(FROZEN_SPEC_PATH), (
            f"frozen_spec.json missing at {FROZEN_SPEC_PATH}"
        )
...
class TestRouteAuthenticationSurface:
    ...
    def test_auth_routes_are_public(self):
        ...
        for route in auth_routes:
            assert route["auth"] == "NONE", (
                f"Expected NONE auth for {route['path']}, got: {route['auth']}"
            )
    ...
    def test_protected_routes_require_cognito(self):
        ...
        protected = [
            ...
            if not r["path"].startswith("/auth/") and r["path"] != "/health"
        ]
        for route in protected:
            assert route["auth"] == "COGNITO", (
                f"Expected COGNITO auth for {route['path']}, got: {route['auth']}"
            )
    ...
    def test_no_custom_lambda_authorizer_routes(self):
        ...
        assert lambda_auth == [], (
            f"Routes still using Lambda authorizer: {lambda_auth}"
        )
...
class TestNoDeprecatedRoutes:
    ...
    def test_deprecated_route_not_deployed(self, deprecated_route):
        ...
        assert deprecated_route not in CANONICAL_ROUTES, (
            f"Deprecated route '{deprecated_route}' still in canonical spec"
        )
    ...
    def test_no_api_prefix_routes_deployed(self):
        ...
        assert api_prefix_routes == [], (
            f"Routes with /api/ prefix found: {api_prefix_routes}"
        )
