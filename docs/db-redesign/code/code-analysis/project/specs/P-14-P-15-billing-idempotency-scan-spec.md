...
- `infra/careervp/api_db_construct.py:164-186` builds the idempotency table (partition key `id`, `expiration` TTL, **no GSI**), but scope-lock says it is empty/unwired. Billing/webhook dedup is a conditional `put_item` on the `id` primary key (`attribute_not_exists(id)`), not an index query — see `subscription_repository.py:279` `record_payment_event`.
- `infra/careervp/api_db_construct.py:304-311` defines the `idempotency-key-index` GSI on the **VPR Jobs table** (`_build_vpr_jobs_table`), queried by `jobs_repository.py:242` `get_job_by_idempotency_key` for *job* dedup. It is NOT on the billing idempotency table and must not be modelled onto it — the billing money path uses primary-key conditional writes.
