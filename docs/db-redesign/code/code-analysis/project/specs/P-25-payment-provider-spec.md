...
- `test_p25_configured_provider_factory_selects_mock_and_stripe` (AC-P25-1): guard the configured-provider factory import inside the test so a missing factory fails on the test's own assertion, not during collection. With `PAYMENT_PROVIDER=mock`, assert the factory returns `MockProvider`. With `PAYMENT_PROVIDER=stripe` and `PAYMENT_PROVIDER_API_KEY_SSM_PARAM=/careervp/test/payment-provider-api-key`, patch the runtime secret resolver to return a generated fixture value, assert the resolver is called with that exact parameter **name**, and assert the factory returns `StripeProvider` configured with the resolved value without placing the value in an environment variable. Parameterize the unsupported configurations as exactly `["placeholder", "bogus"]`; for each, assert `PaymentProviderError.code == "PAYMENT_PROVIDER_CONFIGURATION_ERROR"` so an unsupported live money-path provider fails closed. No real network call.
- `test_p25_devx_billing_lambdas_configure_mock_provider` (AC-P25-1): synthesize the devx stack and locate exactly the billing API Lambda and billing-reconcile Lambda by physical function name. Assert both have `PAYMENT_PROVIDER == "mock"`; both omit `PAYMENT_PROVIDER_PLACEHOLDER`, `PAYMENT_PROVIDER_API_KEY_SSM_PARAM`, and `STRIPE_SECRET_KEY`; and neither environment contains the generated provider-secret fixture value. RED must fail on its own exact environment assertion while both Lambdas synthesize `PAYMENT_PROVIDER == "placeholder"`.
- `test_p25_active_reconcile_uses_configured_provider` (AC-P25-1): against a moto users table containing exactly one `SUBSCRIPTION#CURRENT/status=active` row, one inactive subscription row, and one unrelated row, set `PAYMENT_PROVIDER=mock`, resolve and invoke the synthesized billing-reconcile entrypoint with exactly `{"detail": {"action": "reconcile_subscriptions"}}`, and assert the exact result `{"status": "ok", "checked": 1, "updated": 0, "errors": 0}`. Guard handler resolution and runtime exceptions so the test fails on its own assertion naming the scan/provider failure rather than on an uncaught `ImportError`, `ClientError`, or `NotImplementedError`. Assert no external HTTP call occurs.
...
- `test_p25_mock_webhook_accepts_matching_second_v1` (AC-P25-2): obtain a generated fixture secret at runtime from the `PAYMENT_PROVIDER_WEBHOOK_SECRET` environment name; for the exact raw payload whose `id` is `evt_p25_mock_second_v1` and whose `type` is `checkout.session.completed`, build `t=<now>,v1=<non-matching 64-hex>,v1=HMAC-SHA256(secret, "{t}.{raw_payload}")` so the matching digest is the SECOND `v1` slot, and assert `MockProvider.construct_webhook_event(payload, header, secret)` returns a `WebhookEvent` with `event_id == "evt_p25_mock_second_v1"` and `event_type == "checkout.session.completed"`. This is the MockProvider signing-secret-rotation case and must fail RED with `PaymentProviderError.code == "WEBHOOK_SIGNATURE_VERIFICATION_FAILED"` while the mock keeps only the first `v1`.
