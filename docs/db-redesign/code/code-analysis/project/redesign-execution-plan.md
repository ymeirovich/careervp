...
> 16's `gpt-5-codex`/`gpt-5.3-codex` pin. Human decision: **rule 16 is authoritative.** The draft is
> withdrawn, not merged — every Codex `{model, reasoning}` pair in this file and in spec frontmatter
> uses rule 16's model choice and reasoning-tier tables exclusively. See rule 16 for the full
> rubric (model-choice table, reasoning-tier table, fast decision table); this file does not
> duplicate it.
...
| # | Clause(s) | Step | Claude | Codex | Deps |
|---|---|---|---|---|---|
| 2.0 | P-25 | **Payment-provider port + `MockProvider`** (checkout/portal/verify-webhook/fetch-subscription/list-events). **Mock's `verify_webhook` MUST implement a real HMAC check that REJECTS a tampered signature** (v2.0.0/A6 — so the negative test is meaningful, not tautological); preserves FE checkout/portal URL contract. All 2.1+ billing codes against the port. | opus/high | `gpt-5.3-codex` / high — new port shape + a security-meaningful negative test needs careful sequencing (rule 16 `high`), blast radius contained to the port | 1.* |
| 2.0b | P-25b | **Real `StripeProvider` + signature verification + idempotency negative test — freeze-line, before any *paid* launch** (v2.0.0/A6). Not "deferred, swap later": a paid launch (L-4) must not run untested signature-verification code on the money path. | opus/xhigh | `gpt-5.3-codex` / max — highest cost-of-being-wrong in Wave 2 (real signature verification on the money path, pre-launch freeze-line, clear evaluation criteria) — rule 16 `max` | 2.0 |
| 2.1 | P-14, P-15 | Idempotency wired (webhook + workers, via the port's event id); kill money-path Scan (customer-id → GSI) | opus/high | `gpt-5.3-codex` / high — money-path correctness + a Scan→GSI migration has hidden coupling (existing callers, key design) — rule 16 `high` | 2.0 |
| 2.2 | P-16, P-17, P-18 | Concurrency bounds; `ReportBatchItemFailures`; visibility ≥6× | sonnet/med | `gpt-5.3-codex` / medium — normal implementation across a few files, established SQS/Lambda reliability patterns — rule 16 `medium` | 0.1 |
| 2.3 | P-19 | SFN retry/heartbeat/JitterStrategy FULL | sonnet/med | `gpt-5.3-codex` / medium — same reliability-pattern class as 2.2 — rule 16 `medium` | 0.1 |
| 2.4 | P-20 | Raise API throttle. **Fold in a minimal load/perf harness (v2.0.0 Layer-2 item 3): a locust smoke — hub read + one generate flow, p99 assert — so the throttle target is sized from data, not guessed; emit bootstrap-latency so O-1's D-H8 go/no-go metric actually gets measured (via P-32's correlation-ID/metrics work).** | sonnet/med | `gpt-5.3-codex` / medium — the throttle bump is deterministic, the harness needs some inference (target sizing from data) — rule 16 `medium` | 0.1 |
| 2.5 | P-02 | Fix billing-reconcile handler (`Handler` ≠ `lambda_handler`). *(P-23 canary moved to Wave 1 step 1.0.)* | opus/high | `gpt-5.3-codex` / low — a known, concrete, single-site fix; acceptance criteria are already explicit — rule 16 fast-decision "edit one file / fix one small test" `low` | 0.1 |
| 2.7 | P-31 | EventBridge rule targets get a DLQ (cleanup 1h, reconcile 02:00) | sonnet/med | `gpt-5.3-codex` / low — mechanical IaC wiring onto a well-specified target shape — rule 16 "bulk repetitive edits, obvious pattern" `low` | 0.1 |
