...
| 2.0b-mock | P-25 (mock hardening) | **not started — follow-up.** Discharges B-2-1's fallback: teach `MockProvider` to accept any matching `v1` (mirror StripeProvider). Fresh RED (new mock test) + fresh GREEN. File-isolated from 2.0b-GREEN (`mock_provider.py` vs `stripe_provider.py`), parallel-safe; recommended after 2.0b-GREEN. | Flips B-2-1 to TRUE when landed (GATE re-reads it). | — | — |
...
|---|---|---|---|
| B-2-1 | The mock provider's signature scheme is a faithful stand-in for Stripe's | 2.0, then 2.0b-RED cross-check, then 2.0b-mock | **FALSE (proven 2026-07-25, 2.0b-RED): header/`{t}.{payload}`/300 s MATCH Stripe, but the mock takes only the FIRST `v1` where Stripe accepts ANY matching `v1` (rotation). Currently INERT (no consumer feeds multi-`v1`). Fallback in flight: 2.0b-mock makes the mock conform → flips this TRUE. Does NOT block 2.0b-GREEN.** |
