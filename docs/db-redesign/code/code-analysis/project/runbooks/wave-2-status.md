...
| 2.5a-GREEN | P-02, P-25 | **HUMAN §0.3 REVIEW REQUIRED — not marked done.** The active-subscription scan was repaired locally and its immutable moto assertion now returns exactly `["USER#active"]`. A fail-closed configured-provider factory was added locally, both billing handlers were wired through it, the immutable factory cases pass (`2 passed`), and the immutable active-row reconciliation case passes with the exact result `{"status": "ok", "checked": 1, "updated": 0, "errors": 0}` and zero external HTTP calls. The required compatibility tripwire then exposed a genuine conflict: with `PAYMENT_PROVIDER` and `PAYMENT_PROVIDER_API_KEY_SSM_PARAM` absent, the accepted P-02 resolution test passes but its empty-table scheduled invocation fails solely because the new factory raises `PaymentProviderError(code="PAYMENT_PROVIDER_CONFIGURATION_ERROR")`. Per the prompt, work stopped before any `api_construct.py` edit, full verification, build, synth, diff, deploy, merge, or commit. All five immutable files remain unchanged. The live name-only SSM check returned only the monthly/quarterly price and current/previous webhook-secret parameter names; no value was retrieved and no API-key parameter exists.** | **NEXT/GATE must resolve first:** human §0.3 review must decide how the accepted P-02 invocation supplies the required provider configuration without weakening P-25's fail-closed rule. Do not edit the test, add a permissive default, or resume devx infrastructure wiring until that decision is recorded. Local authorized edits currently exist only in the scan, new factory, and both handlers plus this ledger row; `api_construct.py` is untouched. After a contract-valid resolution and completed GREEN verification, P-02 still owes done-when #3: human-gated deploy only to `CareerVpCrudDevx`, then observe one real scheduled billing-reconcile run in devx logs. | not committed — §0.3 stop; proposed message `fix: repair configured billing reconciliation` | 2026-07-25 |
