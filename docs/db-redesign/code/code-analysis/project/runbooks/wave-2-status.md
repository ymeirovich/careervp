...
| 2.0b | P-25b | **RED landed. Tightened only P-25b's two rule-14 RED-test briefs, then added exactly two tests in `test_p25b_stripe_provider.py`; both collect and fail on their own guarded AC-P25b-1 assertions because `StripeProvider` is absent (not ImportError/collection/fixture failure). B-2-1 cross-check: signed payload and caller-side 300 s tolerance MATCH Stripe, but multiple-`v1` secret-rotation handling DIVERGES, so B-2-1 is FALSE; `ISSUES.md` records the official-doc/SDK evidence and live probe. Existing P-25 mock tests remain `4 passed`; new file ruff+mypy strict clean; scope-diff recognizes P-25b as `test_written`; ZERO files under `src/backend/careervp/` modified.** | **Resolve the newly proven mock rotation gap before continuing:** fresh follow-up RED must add a test where the matching digest is the second `v1`, then a separate fresh GREEN fixes `MockProvider` to accept any matching `v1` (do not edit this session's Stripe RED file). After that, 2.0b-GREEN implements `StripeProvider` against Stripe's real any-matching-`v1` behavior and may not edit `test_p25b_stripe_provider.py`. B-2-2's stable event-id/idempotency negative remains assigned to 2.1. | pending — see commit message in session output | 2026-07-25 |
