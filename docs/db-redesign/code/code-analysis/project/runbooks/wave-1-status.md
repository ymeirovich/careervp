...
| 1.6 | P-07 (delivery) | **not started.** NEW step, added 2026-07-22, runs before 1.1. Deletes the hardcoded dev-pool fallbacks in `src/frontend/lib/pkce.ts:11,15` and `auth.ts:12,16` (verified present 2026-07-22 — a devx build with a missing/typo'd env var currently authenticates against the **dev** pool silently, which is the highest-value single fix in this plan); adds devx Amplify callback/logout URLs to `cognito_construct.py:26-47`; then human-executed: deploy to `CareerVpCrudDevx`, create the Amplify `db-redesign` branch with devx env vars, and perform ONE real login as `ymeirovich@gmail.com` capturing the full wire sequence to `docs/evidence/pkce-devx-verification-*.json`. | — (not started) | — | — |
