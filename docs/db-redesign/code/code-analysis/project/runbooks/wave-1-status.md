...
| 1.6 | P-07 (delivery) | **Code landed; devx deploy done; login proven end-to-end; all seven evidence wires now captured — but 1.6 does NOT close green: two wires FAILED and surfaced a real, unfixed bug.** RED (`08b4211`) then GREEN (`5b42321`): `src/frontend/lib/auth-config.ts` replaces the `?? '<dev value>'` fallbacks with a hard-throw on missing config; `cognito_construct.py` registers the db-redesign callback/logout URLs. Both change-set-deployed to `CareerVpCrudDevx` (P-28 gate, reviewed then executed live this session): Cognito URLs via `p07-16-devx-callbacks` (74 changes, 0 `Replacement:True`), CORS origins via `p07-16-devx-cors-v2` (137 changes, 0 `Replacement:True`) — both confirmed live via `aws` reads post-deploy, not trusted from the diff alone. Amplify `db-redesign` branch created and built (job 1 and job 2 both `SUCCEED`). **A real login as `ymeirovich@gmail.com` against devx was captured via two browser HAR exports and verified end-to-end**: `/oauth2/authorize` (302) → hosted UI (200) → `POST /login` (302, real credentials) → `/callback` (200) → `POST /oauth2/token` (200, real tokens) → `/dashboard` loads and reads real API data. Decoded the returned `id_token`: `iss` = `us-east-1_bAZ6jb6HP` (devx), not the dev pool — the one check that proves environment isolation actually works, not just "a login succeeded somewhere." Full narrative of what broke and how it was found and fixed is in the section below. **Wires 5–7 captured in a follow-up session** (`claude-in-chrome` never reconnected; human logged in manually in the sandboxed Claude Browser pane, agent drove the rest): sign-out (wire 7) **PASSED** — logout clears all `CognitoIdentityServiceProvider.*` localStorage keys, redirects to `/login/`, and a direct nav to a protected route afterward re-redirects to login. But forced-401 and refresh-exactly-once (wires 5–6) **FAILED**: tampering the `idToken` signature and hitting `/prod/users/me`, `/prod/users/me/usage`, `/prod/users/me/subscription`, `/prod/jobs` returned **403 ACCESS_DENIED on every call, never 401**, and zero refresh/oauth2/token network activity occurred across 4 repeated reload attempts — the SPA shows a dead-end "ACCESS_DENIED" banner with a manual, non-refreshing Retry button instead of silently refreshing and retrying. Evidence: `docs/evidence/pkce-devx-verification-20260723T191113Z.json` (real file, not the template). | **1.1 STAYS BLOCKED — now on a substantive finding, not missing evidence.** The 401-refresh-once contract that 1.3c/the soak-reinterpretation section assumed already works does NOT hold for the bad-signature failure mode on devx: the authorizer returns 403 instead of 401, so any refresh-on-401 interceptor (if one exists) never fires, and the user hits a dead end requiring a manual full sign-in. Before unblocking 1.1: (a) decide whether the API Gateway Cognito authorizer should return 401 for invalid signatures, or the frontend should also treat 403 as a refresh trigger, (b) implement and deploy the fix, (c) re-run wires 5–6 and update the evidence file to a passing state. Sign-out (wire 7) needs no further work. | `08b4211`, `5b42321`, `d0c53e2`, `f340fbd`, `65c8d4f` (repo); Cognito risk-config change is AWS-side, no repo diff; evidence file `docs/evidence/pkce-devx-verification-20260723T191113Z.json` pending commit | 2026-07-23 |
...
`passed: false` and `sign_out` marked `passed: true`. This does **not** flip the 1.6 row to fully
closed and does **not** unblock 1.1 — see the 1.6 row above for the concrete next steps.
