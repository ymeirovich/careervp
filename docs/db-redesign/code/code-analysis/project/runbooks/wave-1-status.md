...
| 1.1-RED | P-04, P-05 | **DONE — five tests landed, all failing on their own assertions; zero implementation files touched.** 1.6 confirmed CLOSED first (its row + evidence). Tests: `src/backend/tests/unit/test_p04_p05_auth_idor.py` (P-04 x-user-id scan → RED at `auth_utils.py:44`; P-04 AUTHORIZER_DISABLED scan → RED at `api_construct.py:2130` — **citation drifted from :2106**; P-05 route-matrix ratchet → RED because the shared `extract_user_id` resolver still honours a forged `x-user-id` header; P-05 flat-envelope → RED, denial body is bare `{'error': ...}` missing `classification`/`error_code`/`field`) + `src/backend/tests/integration/test_p05_cross_tenant_idor.py` (cross-tenant IDOR, moto in-process real two-tenant seeding, parametrized over all 9 resource-by-id handlers). Route matrix parsed LIVE from `api_construct.py` route_map (`tests/security/p04_p05_route_matrix.py`); human-readable snapshot at `docs/evidence/p04-p05-route-owner-matrix.md` (40 routes: 6 public / 19 resource-by-id / 15 self-collection). ruff+mypy --strict clean on all new files. **Line citations re-verified & corrected: AUTHORIZER_DISABLED is at `api_construct.py:2130` (not :2106); route_map at `:3275-3347` (not :3251-3323).** | **1.1-GREEN may start in a FRESH session** (must not read this reasoning; may not edit the test files). Two flags for GREEN/human (rule 5): **(1)** the real vulnerability is ONE shared function `extract_user_id` (`auth_utils.py:44`), not 9 per-handler bugs — every handler already owner-checks via claims; the header fallback defeats all of them at once. **(2)** `export_handler` is ALREADY SAFE (reads `claims.sub` directly, never calls `extract_user_id`) — its cross-tenant probe passes today and is a regression guard; GREEN should change nothing in `export_handler`. Post-fix expectation verified: with no claims + no header, all 9 handlers already return 401, so removing the fallback flips every RED case GREEN with no test edits. | `4e16e43` (tests+matrix); ledger hash in follow-up | 2026-07-24 |
| 1.1-GREEN | P-04, P-05 | **DONE — all five 1.1-RED contract tests pass; zero contract-test files touched.** Fresh session (did not see 1.1-RED's reasoning). Three production edits, each matching the RED author's flags: **(1)** `handlers/auth_utils.py` — deleted the single shared client-header identity fallback (was line 44); identity now comes only from validated Cognito claims (the docstring's already-stated contract). This one change flips all 8 cross-tenant integration cases GREEN (forged header → `extract_user_id` returns None → every covered handler fails closed 401) plus the P-04 x-user-id scan and the route-matrix resolver probe. **(2)** `infra/careervp/api_construct.py:2130` — deleted the dead `AUTHORIZER_DISABLED` env var off the cv-tailoring Lambda (zero runtime readers, a re-armable bypass). **(3)** `handlers/job_handler.py` — the existing cross-tenant ownership denial in `get_job` returned a bare `{'error': ...}`; added `_ownership_denied_response()` returning the flat §3 item-10 envelope (403 + `error`/`error_code`/`field`/`classification`, no nested `error.code`) — fixes `test_p05_error_envelope_is_flat`. `export_handler` left untouched (already safe, a regression guard). P-24 NOT implemented and the custom authorizer NOT instantiated (stays dormant); the resolver-failure signal is the existing distinct `logger.warning`, with the CloudWatch metric left to P-24 per AC-P04-2. **VERIFY all green:** backend unit 1356 pass / integration 163 pass (9-route cross-tenant IDOR incl. `export` guard) / frontend F-01 oracle 87 pass / `ENVIRONMENT=devx cdk synth` clean, `AUTHORIZER_DISABLED` absent from every devx template / `cdk diff CareerVpCrudDevx` shows ONE change — `[-] Removed: .AUTHORIZER_DISABLED` (nested stack `[~]` modified, **zero stateful replacement**, RestApi+Cognito byte-stable) / ruff+mypy --strict clean / `validate_naming --verbose` pass. Coverage gate **PASSES (exit 0)**: core branch **55.14%** vs 53.00% enforced_baseline — the 0.06pp gap closed (+2.14pp). scope-diff → P-04 & P-05 `implemented`. `impl_state: implemented` set on both in the yaml (progress bookkeeping, matching the P-06/P-08/P-10 precedent — no contract/invariant/wave change, no version bump). Does anything still SEND `x-user-id`? **No** — swept frontend, `smoke_harness.py`, integration fixtures, and all non-test source: zero senders, so removing the fallback breaks no live caller. | **FLAG for human (rule 5): three PRE-EXISTING, non-contract tests were migrated because they asserted the exact behavior P-04 removes** — they are NOT the 1.1-RED firewall files (those three artifacts are byte-untouched, `git diff` confirms). Each encoded the vulnerability: `tests/unit/test_auth_utils.py` and `tests/unit/test_cognito_middleware.py` asserted a spoofed `x-user-id`/`X-User-Id` header IS honored (now assert fail-closed `None`); `tests/integration/test_l2_auth_integration.py`'s I4 audit asserted the fallback was *contained to* `auth_utils.py` (now asserts the scan finds it in NO handler). Leaving them would have meant keeping the vulnerability to keep them green. Output requirement #3's "zero test files" is satisfied for the CONTRACT tests; these three are flagged here so the human sees them, not buried. Otherwise no drift: implementation matches the prompt and P-04/P-05's scope-lock entries. **1.1 is the last remaining Wave-1 implementation row — the Wave-1 GATE can now run.** | `8c51047` (code+tests+scope-lock+ledger); this ledger-hash line in follow-up | 2026-07-24 |
...
| GATE | — | **PASSED 2026-07-24.** All eight checks adjudicated live this session (git + live AWS + disk), not transcribed. **#1 scope-diff:** all 10 Wave-1 clauses resolve — P-04/P-05/P-06/P-08/P-10 `implemented`, P-07/P-09/P-11/P-22/P-23 `test_written` (P-22 carries its authored spec+test). **#2 both infra dirs green:** `src/backend/tests/infrastructure` 51 passed; `infra/tests/infrastructure` 140 passed — but ONLY after the fix below. On first run the gate was **AMBER**: `test_p64_scratch_path` failed (139 passed/1 failed) because P-06's ARN-scoped `ssm:GetParameter` grant was not scratch-aware and leaked `/careervp/<scratch-env>/jwt-private-key` into the scratch template. The fix already existed on `origin/fix/p06-scratch-ssm-arns` (`c62fb03`, a scratch-aware `_secret_parameter_arns()` returning `[]` in scratch mode, self-verified as a no-op for every non-scratch env). Per the user's decision it was cherry-picked onto `db-redesign` as its own commit (`a555e70`); re-run → 140 passed, `test_p64_scratch_path` 27/27, flipping the gate AMBER→PASS. **#3:** backend unit+integration 1519 passed / 30 skipped / 4 xfailed (cross-tenant IDOR negatives incl. the `export_handler` guard); frontend unit 149 passed (flat-envelope oracle) + integration 87 passed (401 refresh-once contract). **#4 coverage:** enforced_baseline PASSES — core branch **55.14%** vs 53.00, core line 73.00% vs 71.0 (`make coverage-tests` exit 0). Distance to ratchet_target (85/80): core line +12.0pp, branch +24.86pp — report-only. **#5 IMMUTABLE:** `test_p26_blue_green_api.py` 13 passed (RestApi `…Crudservicerestapi5E02FD49` + Cognito `…CognitoUserPool42C0A4E4` logical ids byte-stable); P-24 authorizer DORMANT — `_add_api_authorizer_lambda` (`api_construct.py:2359`) has no call site, the live authorizer is `CognitoUserPoolsAuthorizer` (`:617`, wired `:144`, used by routes `:3180`). **#6:** live `describe-stack-resources CareerVpCrudDevx` = **211** (100 parent + 11 AiAssist + 100 CrudFeatures), `UPDATE_COMPLETE`, well < 400; P-30 smoke evidence `smoke-20260720T203735Z-019ff0.json` 4/4. Old `CareerVpCrudDev` count is a decommission question, not a gate question. **#7:** `docs/evidence/pkce-devx-verification-20260723T204342Z.json` `passed:true`, all seven wires (authorization_redirect, callback_route, token_exchange, authenticated_api_call, forced_401, refresh_exactly_once, sign_out); the soak-reinterpretation rationale is present in this file per RUNBOOK-RULES rule 8. **#8:** P-07b deferral row present in `redesign-execution-plan.md` and marked BLOCKS STAGING; that file's stale "Open" section reconciled in this same commit. | **Open for Wave 2:** P-07b only (COGNITO_ADMIN + implicit-grant removal behind backend proxies, blocks STAGING promotion). Housekeeping: `origin/fix/p06-scratch-ssm-arns` is now merged via `a555e70` and can be closed. **Wave 1 verified — Wave 2 (reliability/money) step 2.0 dependency (1.*) is satisfied; `wave-2-prompts.md` may now be generated (not auto-generated this session).** | `a555e70` (scratch-SSM fix cherry-pick) + this docs-sync commit | 2026-07-24 |
