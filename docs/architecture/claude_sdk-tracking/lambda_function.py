    ...
    with rollup.batch_writer() as batch:
        for agent, data in by_agent.items():
            batch.put_item(Item={
                ...
                "agent_name": agent,
                **_to_decimal(data),
                "self_correction_rate": decimal.Decimal(str(
                    round(data["self_correction_calls"] / max(data["call_count"], 1), 3)
                )),
            })
...
def _scan_day(table, day_start_ms: int, day_end_ms: int) -> list:
    ...
    kwargs = {
        "FilterExpression": (
            Attr("timestamp_ms").between(day_start_ms, day_end_ms - 1)
        )
    }
    while True:
...
def _empty_bucket() -> dict:
    return {
        ...
        "call_count": 0, "self_correction_calls": 0, "failure_count": 0,
    }
...
def _emit_rollup_metrics(date: str, totals: dict, by_agent: dict):
    ...
    metric_data = [
        {"MetricName": "DailyTotalCostMilliUSD",
         "Value": totals["total_cost_usd"] * 1000, "Unit": "None",
         "Dimensions": [{"Name": "Scope", "Value": "Daily"}]},
        {"MetricName": "DailyTotalTokens",
         "Value": totals["total_tokens"], "Unit": "Count",
         "Dimensions": [{"Name": "Scope", "Value": "Daily"}]},
        {"MetricName": "DailyAPICallCount",
         "Value": totals["call_count"], "Unit": "Count",
         "Dimensions": [{"Name": "Scope", "Value": "Daily"}]},
    ]
    for agent, data in by_agent.items():
        metric_data.extend([
            {"MetricName": "DailyAgentCostMilliUSD",
             "Value": data["total_cost_usd"] * 1000, "Unit": "None",
