...
from careervp.dal import get_dal_handler
from careervp.dal.db_handler import DalHandler
from careervp.handlers.models.dynamic_configuration import FeatureFlagsNames
from careervp.handlers.utils.dynamic_configuration import get_configuration_store
from careervp.handlers.utils.observability import logger, tracer
from careervp.logic.utils.idempotency import IDEMPOTENCY_CONFIG, IDEMPOTENCY_LAYER
from careervp.models.input import CreateOrderRequest
from careervp.models.order import Order
from careervp.models.output import CreateOrderOutput
...
def create_order(
    order_request: CreateOrderRequest, table_name: str, context: LambdaContext
) -> CreateOrderOutput:
    IDEMPOTENCY_CONFIG.register_lambda_context(context)  # see Lambda timeouts section

    logger.info(
        "starting to handle create request",
    ...
    order: Order = dal_handler.create_order_in_db(
        order_request.customer_name, order_request.order_item_count
    )
