...
def lambda_context():
    ...
    context.invoked_function_arn = (
        "arn:aws:lambda:us-east-1:123456789:function:cover-letter-handler"
    )
    return context
...
def test_download_url_generated(
    ...
    mock_llm_service.generate_cover_letter.return_value = mock_llm_response
    mock_fvs.validate.return_value = {"valid": True, "warnings": []}
    mock_artifact_storage.generate_presigned_url.return_value = (
...
def test_cv_not_found_propagates(api_gateway_event, lambda_context, mock_dal):
    """
...
def test_vpr_not_found_propagates(
    api_gateway_event, lambda_context, mock_dal, mock_cv_data
):
    """
...
def test_missing_auth_returns_401(lambda_context):
    """
    ...
    event = {
        ...
        "body": json.dumps(
            {"cv_id": "cv-456", "vpr_id": "vpr-789", "job_description": "Test job..."}
        ),
    }
...
def test_rate_limit_returns_429(api_gateway_event, lambda_context, mock_dal):
    """
...
def test_validation_error_format(lambda_context):
    """
...
def test_user_cannot_access_other_user_cv(lambda_context, mock_dal):
    """
