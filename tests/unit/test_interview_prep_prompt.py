...
class TestInterviewQuestionTypes:
    ...
    def test_behavioral_question_detection(self):
        ...
        for question in questions:
            is_behavioral = any(
                phrase in question.lower() for phrase in behavioral_phrases
            )
    ...
    def test_technical_question_detection(self):
        ...
        for question in questions:
            is_technical = any(
                keyword in question.lower() for keyword in technical_keywords
            )
    ...
    def test_situational_question_detection(self):
        ...
        for question in questions:
            is_situational = any(
                phrase in question.lower() for phrase in situational_phrases
            )
...
class TestQuestionPrioritization:
    ...
    def test_gap_questions_prioritized(self, sample_gap_responses):
        ...
        questions = [
            ...
            {
                "text": sample_gap_responses[0]["question"],
                "type": "Gap-Focused",
                "priority": 1,
            },
    ...
    def test_max_questions_limit(self):
        ...
        all_questions = [
            {"text": f"Question {i}", "type": "Technical"} for i in range(15)
        ]
        max_questions = 10
...
class TestAnswerStructureValidation:
    ...
    def test_answer_paragraph_count(self):
        ...
        """
        paragraphs = [
            p.strip() for p in valid_answer.strip().split("\n\n") if p.strip()
        ]
...
class TestAntiAIPatterns:
    ...
    def test_excessive_formality_detection(self):
        ...
        detected_patterns = [
            p for p in formal_phrases if p.lower() in text_with_patterns.lower()
        ]
    ...
    def test_perfect_parallel_structure_detection(self):
        ...
        has_parallel = len(sentences) == 4 and all(s.startswith("I") for s in sentences)
    ...
    def test_no_first_person_recommendation(self):
        ...
        """
        first_person_count = valid_answer.lower().count(
            " i "
        ) + valid_answer.lower().count("i'm")
        assert first_person_count == 0

    def test_answer_tone_variation(self):
        """Test answers have varied tone."""
        formal_answer = (
            "One should always consider the implications of architectural decisions."
        )
        casual_answer = (
            "It's important to think about how your design choices affect the system."
        )
